...
data "template_file" "node-bootstrap" {
  ...
  vars = {
    master_ip = var.master_ip == "" ? var.region_params[var.region]["master_ip"] : var.master_ip
    token     = "${random_id.token-part-1.hex}.${random_id.token-part-2.hex}"
  }
}
...
module "master-mig" {
  ...
  network_ip        = var.master_ip == "" ? var.region_params[var.region]["master_ip"] : var.master_ip
  access_config     = var.access_config
  can_ip_forward    = true
  size              = 1
  compute_image     = var.compute_image
  machine_type      = var.master_machine_type
  target_tags       = [concat([random_id.instance-prefix.hex], var.add_tags)]
  service_port      = 80
...
resource "google_compute_firewall" "vms" {
  ...
  source_ranges = compact(
    [
      "10.128.0.0/9",
      var.subnetwork != "default" ? data.google_compute_subnetwork.subnet.ip_cidr_range : "",
    ],
  )
}
